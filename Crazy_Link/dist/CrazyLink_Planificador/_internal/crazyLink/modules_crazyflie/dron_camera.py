...
class DroneCamera:
    ...
    def capturar_foto(self, posicion: Optional[Tuple[float, float, float]] = None,
        ...
        """
        Captura una foto y la guarda en la carpeta de la sesión actual.

        Args:
            posicion: Tupla (x, y, z) con la posición del dron
            metadata: Diccionario con metadatos adicionales

        Returns:
            Ruta completa de la foto guardada o None si falla
        """
        try:
    ...
    def iniciar_video_corto(self, duracion: int = 10,
                            ...
                            metadata: Optional[dict] = None) -> Optional[str]:
        """
        Inicia la grabación de un video corto de duración fija.

        Args:
            duracion: Duración del video en segundos (default: 10)
